...
const ASSET_IMAGE_MAP = {
  ...
  SUBSTATION: 'https://api.iconify.design/mdi:transmission-tower.svg?color=%236a91ff',
...
export default function NetworkTopologyPage() {
  ...
  const hierarchyAssetIds = useMemo(
    () => new Set(collectHierarchyNodeIds(hierarchyRoots).map((id) => Number(id)).filter(Boolean)),
    [hierarchyRoots]
  );

  const linkableAssets = useMemo(() => {
    if (hierarchyAssetIds.size === 0) return assets;
    return assets.filter((asset) => hierarchyAssetIds.has(Number(getAssetId(asset))));
  }, [assets, hierarchyAssetIds]);
  ...
  const validateForm = () => {
    ...
    if (hierarchyAssetIds.size > 0) {
      if (!hierarchyAssetIds.has(fromAssetId)) {
        return `Source asset #${fromAssetId} is not in hierarchy. Please select a hierarchy asset.`;
      }
      if (!hierarchyAssetIds.has(toAssetId)) {
        return `Target asset #${toAssetId} is not in hierarchy. Please select a hierarchy asset.`;
      }
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const backendMessage =
        ...
        err?.message;
      setError(backendMessage || 'Failed to save topology link');
    }
  };
